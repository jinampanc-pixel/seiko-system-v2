...
function arrangeLabelItems(items: Item[], preset: Preset) { const fields = items.filter(item => item.kind === "field"), codes = items.filter(item => item.kind === "qr" || item.kind === "barcode"), others = items.filter(item => item.kind !== "field" && item.kind !== "qr" && item.kind !== "barcode"), reserve = codes.length ? 12 : 1, usable = Math.max(14, preset.labelW - reserve - 4), columns = fields.length > 5 ? 2 : 1, rows = Math.max(1, Math.ceil(fields.length / columns)), cellW = usable / columns, cellH = Math.min(4.2, (preset.labelH - 4) / rows), arrangedFields = fields.map((item, index) => { const column = Math.floor(index / rows), row = index % rows; return { ...item, x: 2 + column * cellW, y: 2 + row * cellH, w: Math.max(8, cellW - 1), h: Math.max(2.6, cellH - .4), font: clamp(Math.min(item.font, cellH * 1.6), 4, 8) }; }), arrangedCodes = codes.map((item, index) => { const size = Math.min(10, preset.labelH - 4); return item.kind === "qr" ? { ...item, x: preset.labelW - size - 2, y: 2 + index * (size + 1), w: size, h: size } : { ...item, x: 2, y: preset.labelH - 7, w: usable, h: 5 }; }); return [...arrangedFields, ...arrangedCodes, ...others]; }
export function LabelDesigner({ businessId, canManageSizes, order, onBack, initialPurpose = null }: {
    ...
    const [presets, setPresets] = useState<Preset[]>(() => [roll, ...stored<Preset[]>(key(businessId, "presets-v1"), [])]), [presetId, setPresetId] = useState(roll.id), [showSizes, setShowSizes] = useState(false);
    ...
    const dragging = useRef<{
        ...
        x: number;
        y: number;
    } | null>(null), preset = presets.find(p => p.id === presetId) || roll, selectedRecords = records.filter(record => selectedRows.includes(record.id)), printRecords = selectedRecords, current = printRecords[0] || records.find(r => r.id === selectedRows.at(-1)) || records[0] || emptyRecord, selected = items.find(i => i.id === selectedId), products = useMemo(() => [...new Set(records.map(r => r.product))], [records]);
    ...
    const toggleField = (field: string, label: string) => { const existing = items.find(item => item.kind === "field" && item.field === field); if (existing) {
        setItems(all => arrangeLabelItems(all.filter(item => item.id !== existing.id), preset));
        return;
    } setItems(all => arrangeLabelItems([...all, { id: crypto.randomUUID(), kind: "field", field, value: label, fieldLabel: label, showLabel: false, x: 2, y: 2, w: 20, h: 4, font: 7 }], preset)); };
 ...
 <section className="simpleDesigner panel"><div className="simpleDesignerHead"><div><h3>Information on the label</h3><small>Choose what people should see. The preview uses the selected real record.</small></div><button className="secondary" onClick={() => setAdvanced(value => !value)}>{advanced ? "Use simple setup" : "Advanced layout"}</button></div><div className="fieldChecklist">{fieldOptions.filter(option => !option.key.startsWith("field_name:")).map(option => { const item = items.find(entry => entry.kind === "field" && entry.field === option.key); return <div className={`fieldChoice ${item ? "chosen" : ""}`} key={option.key}><label><input type="checkbox" checked={!!item} onChange={() => toggleField(option.key, option.label.replace(/ \(resolved\)$/, ""))}/><span>{option.label.replace(/ \(resolved\)$/, "")}</span></label>{item && <><label className="showName"><input type="checkbox" checked={item.showLabel || false} onChange={event => update(item.id, { showLabel: event.target.checked })}/> Show name</label>{item.showLabel && <input aria-label={`Printed name for ${option.label}`} value={item.fieldLabel || option.label} onChange={event => update(item.id, { fieldLabel: event.target.value })}/>}<div className="fieldEmphasis">{item.showLabel && <label><input type="checkbox" checked={item.labelBold ?? true} onChange={event => update(item.id, { labelBold: event.target.checked })}/> Name bold</label>}<label><input type="checkbox" checked={item.valueBold || false} onChange={event => update(item.id, { valueBold: event.target.checked })}/> Value bold</label></div></>}</div>; })}</div>{purpose === "production" && <label className="customerUpdateChoice"><input type="checkbox" checked={customerUpdates} onChange={event => setCustomerUpdates(event.target.checked)}/><span><b>Allow selected lifecycle stages to send customer updates</b><small>Internal scans remain the source. Only stages enabled later will communicate with the customer.</small></span></label>}<p className="purposeBoundary">{purpose === "inventory" ? "Inventory labels never trigger customer communication." : purpose === "packing" && sourceMode === "order" ? "This whole-order label is scannable evidence of the package contents—not a delivery challan." : purpose === "production" && sourceMode === "person_product" ? "This garment identity remains the same through production, delivery and payment." : ""}</p></section>
 <section className="labelSetup panel"><label><span>{purpose === "production" ? "Production identity" : purpose === "packing" ? "Packing label for" : "Inventory label for"}</span><select value={sourceMode} disabled={!order || purpose === "inventory" || purpose === "production"} onChange={e => setSourceMode(e.target.value as SourceMode)}>{purpose === "production" ? <option value="person_product">One permanent label for each garment</option> : purpose === "packing" ? <><option value="person">Each person’s package</option><option value="group">Grouped outer packages</option><option value="order">Whole order package</option></> : <option value="product">Product / stock group</option>}</select></label><label><span>Label content</span><select value={outputMode} onChange={e => setOutputMode(e.target.value as OutputMode)}><option value="combined">Code + information</option><option value="code">Code only</option><option value="information">Information only</option><option value="custom">Choose myself</option></select></label><label><span>Label size</span><select value={presetId} onChange={e => setPresetId(e.target.value)}>{presets.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</select></label><div className="rollSummary"><b>{preset.labelW} × {preset.labelH} mm · {preset.columns} across</b><small>{preset.outer} + {preset.labelW} + {preset.gapX} + {preset.labelW} + {preset.outer} = {preset.rollW} mm roll</small></div>{canManageSizes && !preset.locked && <button className="iconButton" aria-label="Remove selected size preset" onClick={() => { savePresets(presets.filter(p => p.id !== preset.id)); setPresetId(roll.id); }}>×</button>}</section>
 ...
 {showSizes && <section className="customSize panel"><div><h3>New size preset</h3><p>All measurements are millimetres.</p></div>{Object.entries(newSize).map(([k, v]) => <label key={k}><span>{({ name: "Name", labelW: "Label width", labelH: "Label height", rollW: "Roll width", columns: "Across", outer: "Outer margin", gapX: "Horizontal gap", gapY: "Vertical gap" } as Record<string, string>)[k]}</span><input type={k === "name" ? "text" : "number"} value={v} onChange={e => setNewSize(n => ({ ...n, [k]: e.target.value }))}/></label>)}<button className="primary" onClick={createSize}>Save size</button></section>}
 <div className="labelDesignerGrid"><aside className="labelSidebar panel">
  ...
  <input className="recordSearch" aria-label="Find label records" placeholder="Find person or product" value={recordQuery} onChange={event => setRecordQuery(event.target.value)}/>
 ...
 <section className="printSheet" aria-hidden="true" style={{ gridTemplateColumns: `repeat(${preset.columns},${preset.labelW}mm)` }}>{printRecords.map((r, index) => <div className="printedLabel" key={r.id}>{displayedItems.map(item => <div className={`printedElement element-${item.kind}`} key={item.id} style={{ left: `${item.x}mm`, top: `${item.y}mm`, width: `${item.w}mm`, height: `${item.h}mm`, fontSize: `${item.font}pt`, fontWeight: item.bold ? 700 : 400 }}>{render(item, r, index + 1)}</div>)}</div>)}</section></div>;
}
...
function packingGroupOptions(order: SeikoOrder, groupBy: string): PackingGroup[] { if (groupBy === "product")
    return order.products.filter(product => product.name.trim()).map(product => ({ key: `product:${product.id}`, label: product.name })); const fieldId = groupBy.replace(/^field:/, ""); const field = order.fields.find(item => item.id === fieldId); return [...new Set(order.records.filter(record => !record.held).map(record => String(record.values[`field:${fieldId}`] || "").trim()).filter(Boolean))].map(value => ({ key: `field:${fieldId}:${value}`, label: `${field?.name || "Group"}: ${value}` })); }
...
function orderLabelRows(order: SeikoOrder, source: SourceMode): RecordRow[] { const products = order.products.filter(product => product.name.trim()); const activeRecords = order.records.filter(record => !record.held); const common = { name: "", group: "", size: "", qty: "1", order: order.details.orderNo, client: order.details.clientName }; const commonValues = { order: common.order, client: common.client }; const productValues = (product: SeikoOrder["products"][number], record?: SeikoOrder["records"][number]) => Object.fromEntries(product.specifications.filter(spec => spec.name.trim()).map(spec => { let value = spec.defaultValue; if (spec.role === "asset")
