...
export function LabelDesigner({ businessId, canManageSizes, order, onBack, initialPurpose = null }: {
 ...
 <section className="labelSetup panel">
  <label><span>{purpose === "production" ? "Production identity" : purpose === "packing" ? "Packing label for" : "Inventory label for"}</span><select value={sourceMode} disabled={!order || purpose === "inventory" || purpose === "production"} onChange={e => setSourceMode(e.target.value as SourceMode)}>{purpose === "production" ? <option value="person_product">One permanent label for each garment</option> : purpose === "packing" ? <><option value="person">Each person’s package</option><option value="group">Grouped outer packages</option><option value="order">Whole order package</option></> : <option value="product">Product / stock group</option>}</select></label>
  <label><span>Label type</span><select value={outputMode} onChange={e => setOutputMode(e.target.value as OutputMode)}><option value="combined">Code + information</option><option value="code">Code only</option><option value="information">Information only</option><option value="custom">Choose myself</option></select></label>
  <label className="classificationControl"><span>Classification / group</span>{personDetailFields.length ? <><select aria-label="Classification field for label grouping" value={classificationFieldId} onChange={event => chooseClassification(event.target.value)}><option value="">Choose Person detail…</option>{personDetailFields.map(field => <option key={field.id} value={field.id}>{field.name}</option>)}</select><small>Uses the saved Person-detail value on each label.</small></> : <small className="classificationEmpty">Add a Person detail in Order setup first</small>}</label>
  <div className="managedSizeControl"><span>Label size</span><div className="managedSizeRow"><select value={presetId} onChange={e => setPresetId(e.target.value)}>{presets.map(p => <option key={p.id} value={p.id}>{p.name}</option>)}</select>{canManageSizes && !preset.locked && <button type="button" className="managedRemove" aria-label={`Remove ${preset.name}`} onClick={() => { savePresets(presets.filter(p => p.id !== preset.id)); setPresetId(roll.id); }}>×</button>}</div>{canManageSizes && <button type="button" className="managedAdd" onClick={() => setShowSizes(true)}>+ New size</button>}</div>
