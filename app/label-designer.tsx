...
export function LabelDesigner({ businessId, canManageSizes, order, onBack, initialPurpose = null, initialSourceMode }: {
    ...
    if (order && !purpose)
        return <div className="page labelPurposePage"><section className="labelPurposeHead panel"><div>{onBack && <button className="secondary contextBackButton" onClick={onBack}>← Back to Order</button>}<p className="eyebrow">ORDER {order.details.orderNo}</p><h2>What will this label identify?</h2><p>Choose its use. You can change the information and size next.</p></div></section><section className="purposeGrid"><button onClick={() => choosePurpose("production")}><b>Garment / production</b><span>One identity can follow a garment through work, delivery and payment.</span></button><button onClick={() => choosePurpose("packing")}><b>Packing</b><span>Identify a person’s set, product package or outer order package.</span></button><button onClick={() => choosePurpose("inventory")}><b>Inventory</b><span>Identify stock or batches without creating customer messages.</span></button></section></div>;
    return <div className={`page labelDesignerPage output-${outputMode} ${order ? "" : "noOrder"}`}><section className="labelTopbar panel"><div>{order && onBack && <button className="secondary contextBackButton" onClick={onBack}>← Back to Order</button>}<p className="eyebrow">{order ? `ORDER ${order.details.orderNo} · ${purposeLabel(purpose)}` : "LABEL DESIGNER"}</p><h2>{order ? `${purposeLabel(purpose)} labels for ${order.details.clientName}` : "Design and print labels"}</h2><p>{order ? "The preview and print records come only from the selected orders and products." : "Open an order first. The designer will use only that order's saved people and products."}</p></div><div className="labelHeaderCommandBar">{order && purpose && <button type="button" className="primary labelHeaderSaveSet" disabled={!selectedRows.length} onClick={saveLabelTask}>{activeTaskId ? "Update label set" : "Save label set"}</button>}{order && purpose && <details className="labelHeaderMore" open={labelWorkspaceMoreOpen} onToggle={event => setLabelWorkspaceMoreOpen(event.currentTarget.open)}><summary aria-label="More label actions">•••</summary><div className="labelHeaderMoreMenu"><button type="button" onClick={() => { saveTemplate(); setLabelWorkspaceMoreOpen(false); }}>{activeTemplateId ? "Update layout" : "Save layout"}</button><button type="button" onClick={() => { setOpenLibrary("layouts"); setLabelWorkspaceMoreOpen(false); }}>Saved layouts</button><button type="button" onClick={() => { setOpenLibrary("tasks"); setLabelWorkspaceMoreOpen(false); }}>Saved label sets</button></div></details>}<button className="primary" disabled={!printRecords.length} onClick={print}>Print</button></div></section>
