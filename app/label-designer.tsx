...
export function LabelDesigner({ businessId, canManageSizes, order, onBack, initialPurpose = null, initialSourceMode }: {
 ...
 <div className="labelDesignerGrid"><aside className="labelSidebar panel">
  ...
  <div className="recordList" role="list" aria-label="Available label records">{shownRecords.map(record => <button className={`record ${selectedRows.includes(record.id) ? "selected" : ""}`} data-record-preview={[record.name, record.group, record.values.product_summary || record.product, ...Object.values(record.values).filter(value => value && !String(value).startsWith("{")).slice(0, 8)].filter(Boolean).join(" · ")} key={record.id} onClick={() => { setPreviewId(record.id); setSelectedRows(currentRows => currentRows.includes(record.id) ? currentRows.filter(id => id !== record.id) : [...currentRows, record.id]); }}><span className="check">{selectedRows.includes(record.id) ? "✓" : ""}</span><span><b>{sourceMode === "person" ? record.name : `${record.name}${record.product ? ` — ${record.product}` : ""}`}</b><small>{sourceMode === "person" ? [record.group, record.values.product_summary, record.qty && `Total ${record.qty}`].filter(Boolean).join(" · ") : [record.group, record.size && `Size ${record.size}`, record.qty && `Qty ${record.qty}`].filter(Boolean).join(" · ") || "Order package"}</small></span></button>)}</div>
...
function uniqueSavedNamefunction uniqueSavedName(base: string, names: string[]) { if (!names.includes(base))
...
function normalizedMeasurementValuesfunction normalizedMeasurementValues(order: SeikoOrder, values: Record<string, string | number>) {
