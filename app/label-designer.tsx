...
export function LabelDesigner({ businessId, canManageSizes, order, onBack, initialPurpose = null, initialSourceMode }: {
    ...
    const visibleRecords = records.filter(record => {
        ...
        const matchesFilter = !recordFilterField || !recordFilterValue || (recordFilterField === "contains_product" ? packageProducts(record).includes(recordFilterValue) : recordFilterValueFor(record, recordFilterField) === recordFilterValue);
    ...
    return <div className={`page labelDesignerPage output-${outputMode} ${order ? "" : "noOrder"}`}><section className="labelTopbar panel"><div><p className="eyebrow">{order ? `ORDER ${order.details.orderNo} · ${purposeLabel(purpose)}` : "LABEL DESIGNER"}</p><h2>{order ? `${purposeLabel(purpose)} labels for ${order.details.clientName}` : "Design and print labels"}</h2><p>{order ? "The preview and print records come only from the selected orders and products." : "Open an order first. The designer will use only that order's saved people and products."}</p></div><div className="labelHeaderCommandBar">{order && onBack && <button className="secondary" onClick={onBack}>← Back</button>}{order && purpose && <details className="labelHeaderMore" open={labelWorkspaceMoreOpen} onToggle={event => setLabelWorkspaceMoreOpen(event.currentTarget.open)}><summary aria-label="More label actions">•••</summary><div className="labelHeaderMoreMenu"><button type="button" onClick={() => { saveTemplate(); setLabelWorkspaceMoreOpen(false); }}>{activeTemplateId ? "Update layout" : "Save layout"}</button><button type="button" disabled={!selectedRows.length} onClick={() => { saveLabelTask(); setLabelWorkspaceMoreOpen(false); }}>{activeTaskId ? "Update label set" : "Save label set"}</button><button type="button" onClick={() => { setOpenLibrary("layouts"); setLabelWorkspaceMoreOpen(false); }}>Saved layouts</button><button type="button" onClick={() => { setOpenLibrary("tasks"); setLabelWorkspaceMoreOpen(false); }}>Saved label sets</button></div></details>}<button className="primary" disabled={!printRecords.length} onClick={print}>Print</button></div></section>
