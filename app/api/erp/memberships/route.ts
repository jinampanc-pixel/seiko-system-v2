...
function sanitizeModules(value: Module[] | undefined, role: AccessRole, businessId: string): Module[] {
  const catalog = businessCatalogEntry(businessId);
  const businessModules = new Set<Module>(catalog.allowedModules);
  const preset = ROLE_MODULE_PRESETS[role] || ROLE_MODULE_PRESETS.viewer;
  const source = role === "owner"
    ? [...catalog.defaultModules]
    : Array.isArray(value)
      ? value
      : catalog.defaultModules.filter(module => preset.includes(module));
