...
export function PackingPersonLabelDesigner({ businessId, order, onBack, backLabel }: {
  ...
  return <div className="physicalPackingEditor" data-selected-count={selectedRows.length} data-eligible-count={records.length} onKeyDown={event => {
    ...
    {printReady && <section className="physicalPrintSheet" aria-hidden="true" style={{ width: `${PACKING_OUTPUT.widthMm}mm` }}>{selectedRecords.map(record => <div className="physicalPrintedLabel" key={record.id} style={{ width: `${PACKING_OUTPUT.widthMm}mm`, height: `${PACKING_OUTPUT.heightMm}mm` }}>{items.map(item => <div className="physicalPrintedItem" key={item.id} style={{ left: `${item.x}mm`, top: `${item.y}mm`, width: `${item.w}mm`, height: `${item.h}mm` }}>{renderItem(item, record)}</div>)}</div>)}</section>}
  </div>;
}
