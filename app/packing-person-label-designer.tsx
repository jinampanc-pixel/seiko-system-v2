...
export function PackingPersonLabelDesigner({ businessId, order, onBack, backLabel }: {
  ...
  return <div className="physicalPackingEditor" data-selected-count={selectedRows.length} data-eligible-count={records.length} onKeyDown={event => {
    ...
    {printReady && <section className="physicalPrintSheet" aria-hidden="true" style={{ width: `${PACKING_OUTPUT.widthMm}mm` }}>{selectedRecords.map(record => <div className="physicalPrintedLabel" key={record.id} style={{ width: `${PACKING_OUTPUT.widthMm}mm`, height: `${PACKING_OUTPUT.heightMm}mm` }}><div className="physicalPrintedCanvas" style={{ width: PACKING_OUTPUT.widthMm * MM_PX, height: PACKING_OUTPUT.heightMm * MM_PX }}>{items.map(item => <div className="physicalPrintedItem" key={item.id} style={{ left: item.x * MM_PX, top: item.y * MM_PX, width: item.w * MM_PX, height: item.h * MM_PX }}>{renderItem(item, record)}</div>)}</div></div>)}</section>}
  </div>;
}
