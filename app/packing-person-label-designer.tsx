...
import { eligiblePackingIds, packingQuantities, constrainBox, measureLabelText, MM_PX } from "./lib/packing-label-model";
...
type PackagePresentation = { matchMode?: "all" | "any"; layoutMode: PackageLayoutMode; delimiter: string };
type ProductRule = {
  alias: string;
  included: boolean;
...
const PACKING_ROLL = PHYSICAL_OUTPUT_PROFILES.roll50x25TwoAcross;
...
function loadPackagePresentation(businessId: string, orderId: string): PackagePresentation {
  try {
    ...
    return { matchMode: saved?.matchMode === "any" ? "any" : "all", layoutMode: saved?.layoutMode === "inline" || saved?.layoutMode === "separate" ? saved.layoutMode : "flow", delimiter: saved?.delimiter || DEFAULT_PACKAGE_PRESENTATION.delimiter };
...
export function buildPersonRecords(order: SeikoOrder, rules: PresentationRules, packagePresentation: PackagePresentation, quantities = packingQuantities(order)): PersonRecord[] {
  ...
  const eligible = new Set(eligiblePackingIds(quantities, included, packagePresentation.matchMode));
...
export function PackingPersonLabelDesigner({ businessId, order, onBack, backLabel }: {
  ...
  const predicted = eligiblePackingIds(quantities, draftProducts.map(product => product.id), draftPresentation.matchMode).length;
  ...
  return <div className="physicalPackingEditor" data-selected-count={selectedRows.length} data-eligible-count={records.length} onKeyDown={event => {
    ...
    }}><header><h3>Package contents</h3><button onClick={closeDrawer}>Cancel</button></header><div className="physicalDrawerBody"><label>Include people with<select aria-label="Product matching" value={draftPresentation.matchMode || "all"} onChange={event => setDraftPresentation(old => ({ ...old, matchMode: event.target.value as "all" | "any" }))}><option value="any">Any selected product</option><option value="all">All selected products</option></select></label><p>{draftPresentation.matchMode === "any" ? "Include anyone who ordered at least one checked product. Each person gets one label showing only their ordered products from this selection." : "Include only people who ordered every checked product."} Pieces and people below are totals for the whole order.</p><div className="physicalProductChoices">{order.products.filter(product => product.name.trim()).map(product => {
