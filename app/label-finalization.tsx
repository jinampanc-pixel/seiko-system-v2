...
function enhanceOrderSearch() {
  ...
  input.type = "search"; input.autocomplete = "off"; input.placeholder = "Search order, client, Attn or phone";
  ...
  const render = (query: string) => {
    ...
    const matches = orders.filter(order => [order.details?.orderNo, order.details?.clientName, order.details?.contactPerson, order.details?.contactNumber].filter(Boolean).join(" ").toLowerCase().includes(term)).slice(0, 12);
    ...
    for (const order of matches) {
      ...
      const meta = document.createElement("small"); meta.textContent = [order.details?.contactPerson && `Attn: ${order.details.contactPerson}`, order.details?.contactNumber && `Phone: ${order.details.contactNumber}`].filter(Boolean).join(" · ") || "No contact details saved";
      button.append(title, meta); button.addEventListener("mousedown", event => event.preventDefault());
      button.addEventListener("click", () => { setNativeSelectValue(select, String(order.orderId || "")); input.value = labelFor(order); results.hidden = true; });
...
function currentPreset(): Preset {
  const native = document.querySelector<HTMLSelectElement>(".labelDesignerPage .labelSetup select");
  ...
  try {
    ...
    return presets.find(preset => preset.name === triggerText) || presets.find(preset => preset.id === native?.value) || DEFAULT_PRESET;
...
function ensureCanvasRatio() {
  document.querySelectorAll<HTMLElement>(".labelDesignerPage .labelCanvasPanel").forEach(panel => {
    ...
    const width = Number(match?.[1]) || 50, height = Number(match?.[2]) || 25;
...
function normalizedPrintedLabel(source: HTMLElement) {
  ...
  /* The hidden print sheet already stores the same logical font size as the designer in pt.
     Keep it in pt: 1pt is rendered by browsers as 1.333 CSS px, exactly matching the designer's
     logical font * 1.333px. Converting it through the 8px/mm canvas scale was the mismatch. */
  clone.querySelectorAll<HTMLElement>(".printedElement").forEach(element => {
...
function labelOnlyPrint() {
  ...
  popup.document.open(); popup.document.write("<!doctype html><html><head><title>Preparing labels…</title></head><body>Preparing labels…</body></html>"); popup.document.close();
  window.setTimeout(() => {
    const labels = [...sheet.querySelectorAll<HTMLElement>(".printedLabel")];
    const preset = currentPreset();
    const pitch = preset.labelH + Math.max(0, preset.gapY || 0);
    const rows: string[] = [];
    for (let index = 0; index < labels.length; index += preset.columns) rows.push(`<section class="printRow">${labels.slice(index, index + preset.columns).map(normalizedPrintedLabel).join("")}</section>`);
    const css = `
      ...
      *{box-sizing:border-box;-webkit-print-color-adjust:exact;print-color-adjust:exact}
      ...
      .printedElement{position:absolute!important;display:flex!important;align-items:center!important;overflow:hidden!important;padding:1px!important;line-height:normal!important;white-space:normal!important;user-select:none!important}
    ...
    `;
    popup.document.open(); popup.document.write(`<!doctype html><html><head><meta charset="utf-8"><title>Labels</title><style>${css}</style></head><body>${rows.join("")}</body></html>`); popup.document.close(); popup.focus();
...
export function LabelFinalization() {
  useEffect(() => {
    ...
    enhance();
    const observer = new MutationObserver(schedule); observer.observe(document.body, { childList: true, subtree: true, attributes: true, attributeFilter: ["style", "class"] });
    const click = (event: MouseEvent) => {
      ...
      event.preventDefault(); event.stopPropagation(); event.stopImmediatePropagation(); labelOnlyPrint();
    };
